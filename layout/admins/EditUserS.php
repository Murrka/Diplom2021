<?php
    ...
    $id=$_GET['id'];
    $UserResult=mysqli_query($bd, "SELECT * FROM `users` INNER JOIN `users_students` on users.users_name=users_students.students_log WHERE users.users_id='$id'");
    ...
    <body>
        ...
        <section class="SectionBodyHome">
            ...
            <div class="DivContentHome">
                ...
                <div class="AddUser">
                    <form action="../../php/ScriptEditUserS.php" method="POST">
                        
                        <div><p>Фамилия студента</p>
                        <input type="text" name="fam" value="<?=$userRow['students_f'];?>">
                        </div>
                        <div><p>Имя студента</p>
                        <input type="text" name="name" value="<?=$userRow['students_n'];?>">
                        </div>
                        <div><p>Отчество студента</p>
                        <input type="text" name="otch" value="<?=$userRow['students_o'];?>">
                        </div>
                        <div>
                        <p>Выберите группу</p>
                        <select name="Groups" id="AddUserSelect">
                            <?php include('../../php/autoZapol.php'); 
                                for($j=0; $j<count($groups); $j++){
                                    if($groups[$j]==$userRow['students_group']){
                            ?><option value="<?=$groups[$j];?>" selected><?=$groups[$j];?></option>
                            <?php }
                                else{?>
                        ...
                        <div><p>Логин пользователя</p>
                            <input type="text" name="login" value="<?=$userRow['users_name']?>" readonly>
                        </div>
                        <div><p>Введите новый пароль пользователя</p>
                            <input type="password" name="password">
                        </div>
                        <div><p>Введите Pin-code администратора для внесения изменений</p>
                            <input type="password" name="pincodes">
                        </div>
                        <div>
                            <input type="submit" name="SaveUser1" value="Изменить профиль студента">
                        </div>
