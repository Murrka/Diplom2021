<?php
    ...
    $result=mysqli_query($bd, "SELECT * FROM `tasks` WHERE id_SL='$id_SL' and catt_tasks='Практические занятия' ORDER BY data_create");
    ...
    <body>
        ...
        <section class="SectionBodyHome">
            ...
            <div class="DivContentHome">
                <div class="Zagolovok">
                    <h1><?=$disResult['name_dis']?>>Практические занятия</h1>
                </div>
                <div class="DivHBoxs">
                    <?php
                        while($tasksResult=mysqli_fetch_array($result,MYSQLI_ASSOC)){
                            if(!empty($tasksResult['id_task'])){ ?>
                                <a href="Tasks.php?id_SL=<?=$id_SL?>&id_task=<?=$tasksResult['id_task']?>" id="Catt" class="Catt"><div id="HBox">
