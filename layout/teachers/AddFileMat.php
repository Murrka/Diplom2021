<?php
    ...
    $dis=$_GET['dis'];
    $id_SL=$_GET['id_SL'];
    $resultAllTask=mysqli_query($bd, "SELECT * FROM `tasks` WHERE id_SL='$id_SL' ORDER BY data_create");
?>
    ...
    <body>
        ...
        <section class="SectionBodyHome">
            ...
            <div class="DivContentHome">
                <div class="Zagolovok">
                <h1><?=$_GET['gr'];?>><?=$_GET['dis'];?>>Методический материал>Добавить файл</h1>
                </div>
                <div class="FormFileAdd">
                    <form action="../../php/AddMetFile.php?gr=<?=$_GET['gr']?>&dis=<?=$_GET['dis']?>&id_SL=<?=$_GET['id_SL']?>" method="POST" enctype="multipart/form-data">
                        <div>
                        <p>Выберите Задание</p>
                        <select name="Task" id="AddUserSelect">
                            <?php  
                                while($rowAllTasks=mysqli_fetch_array($resultAllTask, MYSQLI_ASSOC)){
                            ?><option value="<?=$rowAllTasks['id_task'];?>"><?=$rowAllTasks['name_tasks'];?></option>
                            <?php } ?>
                        </select>
                        </div>
                        <p>Выберите файл</p>
