<?php
    ...
    $group=$_GET['gr'];
    $dis=$_GET['dis'];
    $result=mysqli_query($bd, "SELECT id_task, name_tasks, data_create, study_load.id_SL FROM `tasks` INNER JOIN `study_load` on tasks.id_SL=study_load.id_SL WHERE study_load.name_group='$group' and study_load.name_dis='$dis' and tasks.catt_tasks='Теоритические занятия' ORDER BY data_create");
    ...
    <body>
        ...
        <section class="SectionBodyHome">
            ...
            <div class="DivContentHome">
                <div class="Zagolovok">
                <h1><?=$_GET['gr'];?>><?=$_GET['dis'];?>>Теоритические занятия</h1>
                </div>
                <div class="DivHBoxs">
                    <?php
