<?php
//редактирование профиля teachers
    session_start();
    ...
    if(empty($fam) or empty($name)){
        ...
        header('Location: ../layout/admins/EditUserT.php?id='. $id);
        exit();
    }
    if(empty($pin)){
    ...
    if($rowPin['admins_pin']==$pin){
        ...
        $result2_1=mysqli_query($bd, "UPDATE `users_teachers` SET teachers_f='$fam', teachers_n='$name', teachers_o='$otch', teachers_otdel='$otdel' WHERE teachers_log='$login'");
